#-----------------------------------------------------------------------------
# process_travis_voter_rosters_2025_11_04.py
#
...
#-----------------------------------------------------------------------------
"""process_travis_voter_rosters_2025_11_04.py""" # for pylint
...
def process_excel_workbook(pathname, ballot_type, vote_date):
    ...
    while cur_row < num_rows:
        ...
        if num_columns == 3:
            precinct = excel_data.iat[cur_row, 0]
            vuid = excel_data.iat[cur_row, 1]
            name = excel_data.iat[cur_row, 2]
            words = name.split(r',')

            # Setup the name headings
            if cur_row == 2:
                ...
                if len(words) != 2:
                    print(f"Voter name parse error: {name}")
                    ret_val = False
                    return ret_val
                first_name = words[0]
                last_name = words[1]
