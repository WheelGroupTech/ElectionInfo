...
Python scripts for processing and analyzing election data: ES&S cast vote records
(CVRs), scanner results tapes, Travis County voter registration and roster files,
and Travis Central Appraisal District (TCAD) property exports.

Most tools are plain Python scripts run from a terminal. Several folders include
their own detailed `README.md` files with setup steps, expected inputs, and sample
commands.

> Looking for the compiled desktop tools instead? See
> [`../Applications/README.md`](../Applications/README.md) (Election Explorer).
