 ...
 *
 * v1 (Phases 1-2): values are emitted as stored -- text as-is, numbers as their
 * stored decimal, dates as serials, t="b" as TRUE/FALSE, t="e" as the error
 * text. Style-driven date/number formatting is a later phase.
 */
...
static BOOL cell_value(const char *tag,
                       const char *tagend,
                       const char *cellend,
                       const SharedStrings *shared,
                       RowBuilder *rb,
                       ...
                       const char **shared_ptr)
{
    ...
    *shared_ptr = "";
    ...
    /* everything else reads <v>...</v> */
    {
        ...
        {
            /* str / e / n: emit the value text (unescape covers str/e) */
                                    ...
                                    size_t errcch)
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            if (cgt[-1] != '/')
            {
                if (!cell_value(cell, cgt, cellend, ctx->shared, &rb, col, &is_shared, &sptr))
                {
                    status = EeLoadStatus_Error;
                              ...
                              size_t error_cch)
{
    ...
    size_t file_size = 0;
    mz_zip_archive zip;
    char *wb = NULL, *rels = NULL, *shared_xml = NULL, *sheet_xml = NULL;
    size_t wb_size = 0, rels_size = 0, shared_size = 0, sheet_size = 0;
    SheetRef sheets[EE_XLSX_MAX_SHEETS];
